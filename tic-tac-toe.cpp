...
double exec(struct board *current, struct board *oldstate,int oldact,struct board *start)
{
    ...
    int act2=getaction(newb);

    //if act is filled state, assumed as loss
    if(newb->state[act2]!=-1)
    {
        updateQsa(newb,act2,-1);
        return 1;
    }

    int *state2=new int[9]; 
    copystate(newb,state2);

    state2[act2]=0;

    printstate(state2);

    if(checkwin(state2,0))
    {
        updateQsa(newb,act2,1);
        updateQsa(current,act,-1);
        return -1;
    ...
    struct board *newb2=search(start,state2);

    double val=exec(newb2,newb,act2,start);
    updateQsa(current,act,0.8*val);
    updateQsa(newb,act2,-0.8*val);
    return 0.8*val;
}
...
int main()
{
    ...
    double val=exec(newgame,0,0,start);
    cout<<endl;
    cout<<endl;
    cout<<endl;
    if(val<=0)
